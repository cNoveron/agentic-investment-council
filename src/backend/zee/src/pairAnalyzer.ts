...
const pairAnalyzer = new Agent({
    ...
    instructions: [
        ...
        "Return pairs in a standardized JSON array format",
        "Only consider known token symbols",
        "Ignore irrelevant text and focus on token symbols",
    ],
});

async function getPoolAddress(tokenA: string, tokenB: string): Promise<string | null> {
...
(async function main() {
    // Example usage
    const state = StateFn.root(pairAnalyzer.description);
    state.messages.push(
...
export class PairAnalyzer {
    ...
    async extractPairs(state: any) {
        ...
        // Parse the comma-separated string into an array of pairs
        ...
        for (const pair of pairs) {
            ...
            if (TOKEN_ADDRESSES[token0 as keyof typeof TOKEN_ADDRESSES] &&
                ...
                poolAddresses[pair] = poolAddress as string;
            } else {
